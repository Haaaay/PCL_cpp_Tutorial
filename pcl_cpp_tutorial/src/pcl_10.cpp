...
void calc_normal(KDTree &kdtree, pcl::PointCloud<pcl::PointXYZ>::Ptr &src, int K, pcl::PointCloud<pcl::Normal>::Ptr &cloud_normal){
	//KdTree 세팅
	...
	for(int i=0; i<num_pc; ++i){
		...
		//SVD에서 가장 작은 eigenvalue와 대응되는 eigenvector가 그 인근 pointcloud에서 추출한 normal vector
...
int main(int argc, char **argv){
	...
	if(pcl::io::loadPCDFile<pcl::PointXYZ>("/home/maniac/velo_ws/src/pcl_cpp_tutorial/naverlabs_vel16.pcd", *raw_cloud)==-1){
		PCL_ERROR("Couldn't read source pcd file!\n");
		return(-1);
	}
	//Voxelization 후에 Transformation 처리하는 것이 연산 효율적이다.
	...
	*raw_cloud=*cloud_voxel;
	...
	int level=7;
	float scale=0.7;
	viewer.addPointCloudNormals<pcl::PointXYZRGB, pcl::Normal>(cloud_color, cloud_normal, level, scale, "pc_w_normal", 0);
	viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 100, "pc_w_normal");
